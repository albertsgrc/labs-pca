...
void vector_add(TIPUS *A, TIPUS *B, TIPUS *C, int len, int N_iter)
{
    ...
    size_t stride = sizeof(__m128i)/sizeof(TIPUS);

    for (j=0; j<N_iter; j++)   
	for (; A<Af; A += stride, B += stride, C += stride) {
            __m128i pa, pb, pc;

            pa = _mm_load_si128((__m128i*) A);
            pb = _mm_load_si128((__m128i*) B);
            pc = INTR(pa, pb);
            _mm_store_si128((__m128i*) C,pc);
        }
}
